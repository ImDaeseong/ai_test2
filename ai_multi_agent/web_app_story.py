...
from main import find_project, project_dirs, run_prompt_once, story_managed_output_file, story_managed_summary
...
def _prompt_file(project: Path, chapter_num: int) -> Path:
    return project / "chapters" / f"ch{chapter_num:03d}_GPT_프롬프트.md"


def _prompt_items(project: Path) -> list[dict]:
    summary = story_managed_summary(project)
    items = []
    for chapter_num in range(1, int(summary["total"] or 0) + 1):
        prompt_file = _prompt_file(project, chapter_num)
        output_file = story_managed_output_file(project, chapter_num)
        items.append({
            "num": chapter_num,
            "label": f"ch{chapter_num:03d}",
...
def _output_items(project: Path) -> list[dict]:
    ...
    for item in _prompt_items(project):
        if item["done"]:
            output_file = story_managed_output_file(project, item["num"])
            items.append({
                "label": item["label"],
...
def _list() -> list[dict]:
    result = []
    for project in project_dirs(STORY_OUTPUT_DIR):
        summary = story_managed_summary(project)
        result.append({
            "name": project.name,
            "title": summary["title"],
...
def _detail(name: str) -> dict | None:
    project = find_project(STORY_OUTPUT_DIR, name)
    if not project:
        return None
    summary = story_managed_summary(project)
    ...
    return {
        ...
        "next_label": f"ch{summary['next_num']:03d}" if summary["next_num"] else None,
...
def _run_chapter(project: Path, chapter_num: int, force: bool, max_tokens: int) -> Path:
    prompt_file = _prompt_file(project, chapter_num)
    if not prompt_file.exists():
        raise FileNotFoundError(f"프롬프트 파일이 없습니다: {prompt_file.name}")
    output_file = story_managed_output_file(project, chapter_num)
    return run_prompt_once(prompt_file, output_file, "story", force=force, max_tokens=max_tokens)
...
def api_run():
    ...
    try:
        ...
        try:
            saved = _run_chapter(project, int(chapter_num), force=force, max_tokens=max_tokens)
        finally:
            RUN_LOCK.release()
...
<title>Story Prompt Runner</title>
<style>
:root{--bg:#111318;--panel:#1a1d24;--panel2:#222733;--line:#333947;--text:#eef1f7;--muted:#9aa3b5;--accent:#68d391;--ok:#4fc07a;--warn:#f0aa4f;--bad:#ef5f6b;--font:"Malgun Gothic","Apple SD Gothic Neo",sans-serif}
...
.btn-row{display:flex;gap:8px;margin-top:14px;flex-wrap:wrap}button{font-family:var(--font);border:1px solid var(--line);border-radius:8px;padding:8px 12px;font-size:12px;font-weight:700;cursor:pointer;background:var(--panel2);color:var(--text)}button.primary{background:var(--accent);color:#07201a;border-color:var(--accent)}button.warn{background:var(--warn);color:#201204;border-color:var(--warn)}button:disabled{opacity:.45}.status{margin-top:12px;font-size:13px;color:var(--muted);white-space:pre-wrap}.status.ok{color:var(--ok)}.status.err{color:var(--bad)}.result-panel{border-color:var(--ok)}.spinner{display:inline-block;width:14px;height:14px;margin-right:6px;border:2px solid var(--muted);border-top-color:var(--accent);border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle}@keyframes spin{to{transform:rotate(360deg)}}
</style>
</head>
<body>
<header><h1>Story Prompt Runner</h1><span class="sub">ai_story/output · all chapter prompts · port 5400</span></header>
<div class="layout"><aside class="sidebar"><div class="sidebar-hd">Story Projects</div><div class="sidebar-body" id="proj-list">Loading...</div></aside><main class="content" id="content"><div class="empty-state">Select a project.</div></main></div>
<script>
let cur=null;
