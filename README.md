...
> 총 7개 소스 프로젝트 수록. AI 뮤직비디오 프롬프트 생성, 애니메이션, CapCut 편집 자동화, 멀티 에이전트 실행, 유튜브 리서치, 로컬 음악 분석, 채용 적합도 분석 도구 모음.
> 검증 이력·테스트 결과: [`VERIFICATION.md`](./VERIFICATION.md)

## 저장소 목표

개인 프로젝트 작업공간입니다(실제 경로는 PC마다 다름). 이 문서는 전체를 훑어보기 위한 요약 인덱스이며, 프로젝트별 상세 기능·명령어는 각 폴더의 `README.md`/`CLAUDE.md`를 참조하세요.

구조 변경·리팩터링 전에는 루트 governance 문서를 먼저 확인: [`SPEC.md`](./SPEC.md)(목적·범위·완료 기준) → [`ARCHITECTURE.md`](./ARCHITECTURE.md)(구조 위험·모듈 경계) → [`SECURITY_BOUNDARY.md`](./SECURITY_BOUNDARY.md)(API 키·안전 경계) → [`HOLD_CONDITIONS.md`](./HOLD_CONDITIONS.md)(중단 조건) → [`ROADMAP.md`](./ROADMAP.md)(현대화 순서).

---

## 환경 설정

API 키가 필요한 프로젝트는 `.env.example`을 `.env`(또는 `.env.local`)로 복사한 뒤 키를 입력하세요.
...
> 나머지 5개 프로젝트는 외부 API 없이 로컬 파일 처리만 수행합니다. `.env*` 파일은 `.gitignore`에 등록되어 Git에 커밋되지 않습니다.
...
3개 프로젝트가 MV 제작 파이프라인을 이루고, 나머지 4개(`ai_anime`·`youtube_research`·`music_insight_studio`·`CareerDiff`)는 독립 실행됩니다.
...
`ai_multi_agent`는 이 저장소의 `ai_img_video_aiBoygirl`·`ai_anime` 프롬프트뿐 아니라 별도 저장소 `ai_test1`의 `ai_story`/`ai_Scenario`/`ai-webtoon` 프롬프트도 실행합니다(웹 UI 5개 중 3개는 저장소 밖 프로젝트) — 위 7개 프로젝트 범위와 혼동 주의.

### 주요 미완성/HOLD 항목

- **ai_multi_agent**: 5개 웹 UI의 중복 로직 통합 작업 진행 중 — story/scenario는 공유 스캐폴드(`web_app_scaffold.py`)로 추출 완료(2026-07-13), anime/mv/webtoon은 도메인 모델이 달라 아직 미착수.
...
- 대부분 **로컬 실행 우선** 설계 — 외부 클라우드 API는 `ai_multi_agent`(필수)·`CareerDiff`(선택, mock 대체 가능)만 사용
...
- 스택: Python 3.9+ 5개(`ai_multi_agent`·`youtube_research`는 3.8+), Next.js + TypeScript 1개(`CareerDiff`) — 프로젝트별 `requirements.txt`/`package.json` 또는 설치 안내 보유
- 테스트: `ai_anime` 77개·`ai_img_video_prompt_capcut` 65개·`ai_multi_agent` 40개·`youtube_research` 37개(모두 `pytest tests_unit.py -q`) · `ai_img_video_aiBoygirl` 337개(`pytest -q`) · `music_insight_studio` 34개(`.venv\Scripts\python.exe -m unittest discover -s tests`) · `CareerDiff` 116개(`cd CareerDiff/app && npm test`, vitest)
