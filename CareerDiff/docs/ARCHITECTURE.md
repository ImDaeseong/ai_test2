...
- 검증 케이스: 프로젝트 `data/<UUID>.json` — `candidateProfile` 원문은 넣지 않고
  `candidateProfileHash`(sha256)만 저장한다. 같은 이력서로 여러 공고를 분석하면 프로필 텍스트가
  케이스마다 그대로 반복 저장되어(실측: 175개 케이스 중 실제 다른 이력서는 3종, 중복 저장량만
  약 1.9MB) 낭비였던 것을 수정 — 원문은 `data/profiles/<hash>.json`에 내용 기준으로 한 번만
  저장하고, 케이스는 해시로 참조한다. 과거 이력서가 그 시점 그대로 남아야 하는 재현성(같은 입력 →
  같은 결과, 회귀 테스트 용도)은 유지된다 — 해시가 내용을 그대로 가리키기 때문이다.
  `candidateProfile` 원문이 그대로 박혀 있는 예전 형식 파일도 읽기는 계속 지원한다(마이그레이션
  전 파일과의 하위호환).
- 기존 데이터를 새 형식으로 옮길 때는 `scripts/dedupe-profiles.mjs [data-디렉터리]`를 실행한다.
  이미 마이그레이션된 파일은 건너뛰고, 각 파일은 해시로 원문을 되읽어 정확히 일치하는지 확인한
  뒤에만 덮어써 재실행해도 안전하다.
