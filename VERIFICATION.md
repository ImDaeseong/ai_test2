...
The `C:\path\to\` prefix below replaces the original machine-specific home path in historical command examples.

Run from each project directory:

```powershell
cd C:\path\to\ai_test2\ai_anime
python -m pytest tests_unit.py -q

cd C:\path\to\ai_test2\ai_img_video_aiBoygirl
python -m pytest -q

cd C:\path\to\ai_test2\ai_img_video_prompt_capcut
python -m pytest tests_unit.py -q

cd C:\path\to\ai_test2\youtube_research
python -m pytest tests_unit.py -q
...
- `python -m pytest -q --basetemp C:\path\to\hermes-agents\.tmp_ai_test2_pytest\aiBoygirl` in `ai_img_video_aiBoygirl` -> `337 passed, 1 warning in 4.67s`.
- `python -m pytest tests_unit.py -q --basetemp C:\path\to\hermes-agents\.tmp_ai_test2_pytest\prompt_capcut` in `ai_img_video_prompt_capcut` -> `65 passed, 1 warning in 0.38s`.
- `python -m pytest tests_unit.py -q --basetemp C:\path\to\hermes-agents\.tmp_ai_test2_pytest\multi_agent` in `ai_multi_agent` -> `27 passed, 1 warning in 0.22s`.
- `python -m pytest tests_unit.py -q --basetemp C:\path\to\hermes-agents\.tmp_ai_test2_pytest\youtube_research` in `youtube_research` -> `37 passed, 1 warning in 0.24s`.
